...
export class UsersService {
  ...
  addTeamMember(data: { email: string, firstName: string, lastName: string, role: string }): Observable<any> {
    return this.http.post<any>(`${environment.apiEndpoint}/user/addTeamMember`, data)
      .pipe(map(response => {
        return response;
      }));
  }

  getTeamLimits(): Observable<any> {
    return this.http.get<any>(`${environment.apiEndpoint}/user/getTeamLimits`)
      .pipe(map(response => {
        return response;
      }));
  }
