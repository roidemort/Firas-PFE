...
// Add this method to get indicators
getIndicators(): number[] {
  const totalSlides = Math.ceil(this.cards.length / this.cardsToShow);
  return Array.from({ length: totalSlides }, (_, i) => i);
}

// Add this method to get indicator index
getIndicatorIndex(index: number): number {
  return Math.floor(this.currentIndex / Math.ceil(this.cardsToShow));
}

// Add this method to navigate to specific indicator
goToIndicator(index: number): void {
  const slideIndex = index * Math.ceil(this.cardsToShow);
  this.currentIndex = Math.min(slideIndex, this.cards.length - this.cardsToShow);
}
}
